...
namespace Baozipayment.Models
{
	...
	class PaymentInfo
	{
		...
		public PaymentStatus? paymentStatusValue
		{
			get
			{
				return tryParseEnum<PaymentStatus>(payment_status);
			}
		}
		...
		public decimal? grossValue
		{
			get
			{
				return tryParseDecimal(mc_gross);
			}
		}
		...
		private static EnumType? tryParseEnum<EnumType>(string raw) where EnumType : struct, IConvertible
		{
			Debug.Assert(typeof(EnumType).IsEnum);
			Type typeInfo = typeof(EnumType);
			var query = from en in (Enum.GetValues(typeInfo) as EnumType[])
						where (Attribute.GetCustomAttribute(typeInfo.GetField(en.ToString()), typeof(DescriptionAttribute)) as DescriptionAttribute).Description == raw
						select en;
			return query.SingleOrDefault<EnumType>();
		}
		#endregion
